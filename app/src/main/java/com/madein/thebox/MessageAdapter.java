...
public class MessageAdapter extends RecyclerView.Adapter<MessageAdapter.MessageHolder> {
   ...
   private Context context;
    ...
    public void onBindViewHolder(@NonNull MessageHolder holder, int position) {
        ...
        if (messages.get(position).getSender().equals(FirebaseAuth.getInstance().getCurrentUser().getEmail())){ //if we are sender
            Glide.with(context).load(senderImg).error(R.drawable.logoicon).placeholder(R.drawable.logoicon).into(holder.profimage);
            ...
            Glide.with(context).load(receiverImg).error(R.drawable.logoicon).placeholder(R.drawable.logoicon).into(holder.profimage);
            ConstraintSet constraintSet=new ConstraintSet();
            constraintSet.clone(constraintLayout);
